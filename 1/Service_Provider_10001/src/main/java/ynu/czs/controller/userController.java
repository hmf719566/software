...
public class userController {
    ...
    static {
        users.add(new User(1, "zhangsan from 10001", "123456"));
    }
    ...
    public User updateUser(
            @PathVariable("userId") Integer userId,  // 显式指定路径变量名称
    ...
    public String deleteUser(
            @PathVariable("userId") Integer userId // 显式声明路径变量名称
    ) {
        boolean exists = users.stream().anyMatch(u -> u.getUserId().equals(userId));
        if (!exists) {
            throw new RuntimeException("用户不存在"); // 增加存在性校验
        }
